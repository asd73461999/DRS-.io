...
class IOUMetric:
    ...
    # 修正方法定义，添加self参数
    def get_iOu(self, mask_name, predict):
...
#Since the mask image is a black-and-white grayscale image, those less than 125 can be regarded as black
        ...
        # 确保预测图像和掩码图像具有相同尺寸(576,576)
        if image_mask.shape != (576, 576):
            image_mask = cv2.resize(image_mask, (576, 576))
        if predict.shape != (576, 576):
            predict = cv2.resize(predict, (576, 576))
...
def get_dice(mask_name, predict):
    ...
    # 修复除零错误
    dice_sum = predict.sum()+image_mask.sum()
    if dice_sum == 0:
        dice = 1.0
    else:
        dice = (2. *intersection) / dice_sum
    # 确保返回标量值而不是数组
    return float(dice)
